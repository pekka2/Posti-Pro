...
class ModelExtensionShippingSmartPost extends Model {
	function getQuote($address) {
		...
		$weight = $this->cart->getWeight();
         ...
         if ($status) {
         	if($this->config->get('smartpost_top')){
         		$top = $this->config->get('smartpost_top');
         	} else {
         		$top = 5;
         	}
        ...
        if($address['iso_code_2'] == 'FI'){
         ...
         if($total  >= $this->config->get('smartpost_cargo_sum')){
              if($this->config->get('smartpost_free_cargo')){
                  $cost = 0;
                  $koti = 0;
                  $rahtivapaa = $this->language->get('text_free_cargo');
              }elseif($this->config->get('smartpost_discount_cargo') && $this->config->get('smartpost_discount_cargo_percent')){
                  $summa = ($cost/100)*$this->config->get('smartpost_discount_cargo_percent');
                  $cost = round($cost-$summa,2);
                  $koti = 5;
                  $ale = '(' . $this->language->get('text_discount') . ' ' . $this->config->get('smartpost_discount_cargo_percent') . '%) ';
              }
         }

         if($status){

				$url = 'https://locationservice.posti.com/location?&types=SMARTPOST&types=LOCKER&locationZipCode=' . str_replace(' ', '', $address['postcode']) .'&top=' . $top;
          ...
          $response = json_decode($result);
               ...
               foreach($response->locations as $place){
						...
						$quote_data[$place->pupCode] = array(
							...
							'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('smartpost_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
						);
                }
          }
      }
	...
	protected function cost($weight){
		    foreach($this->config->get('smartpost_hinnasto') as $result){
			    if($weight <= $result['kg'] ){
				    return $result['price'];
			    }
		    }
